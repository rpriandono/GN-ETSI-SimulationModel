...
uint32_t RoundRobin[1000];				 // for Round Robin strategy start transceiver [vehicle ID]
uint32_t RobinIncrease[1000]; 			 // for Round Robin strategy increase or decrease [vehicle ID]
/*=================================*/
	...
	if (Simulator::Now() <= Chronos){
		...
		std::cout<<(avgDropCCH/(numVehicles*numOfPackets))*100<<", "
		  ...
		  for(int i=0;i<numVehicles;i++){
			  for(int j=0;j<numVehicles;j++){
				  if(i!=j){
					  InfoAge[i]+= Age[i][j]/ReceivePktCounter[i][j];
					  //std::cout<<Age[i][j]<<", "<<ReceivePktCounter[i][j]<<std::endl;
				  }
			  }
			  InformationAge+= InfoAge[i]/(numVehicles-1);
			  //std::cout<<InfoAge[i]<<std::endl;
		  }
...
void RoundRobinInit(uint32_t car){
	boost::random::mt19937 gen;
	int seeder = (int)time(NULL)+(car+1);
	gen.seed(seeder);
	boost::random::uniform_int_distribution<> trans(1, 3);
	boost::random::uniform_int_distribution<> increase(1, 2);
	RoundRobin[car]=trans(gen);
	RobinIncrease[car]= increase(gen);
}
...
void ReceivePacket (Ptr<Socket> socket)
{
	...
	while (packet = socket->RecvFrom(from))
    {
		...
		/* ===================== Merge table does not used in Round Robin ====================
  ...
  if (pktCount <= MaxPkt)
    {
	  ...
	  if(tempPktCount[iCarID]!=pktCount){
		  ...
		  // ========== Round Robin ==========
		  uint32_t RR;
		  if (RobinIncrease[iCarID]==1){ RR = (RoundRobin[iCarID]-1)+pktCount;}
		  else { RR = (RoundRobin[iCarID]-1)+((MaxPkt+1)-pktCount);}
...
int main (int argc, char *argv[])
{
  ...
  double CBRmonirotrInterval = 10.0;	// interval time (millisecond) monitor CBR calculation TS 102 636-4-2 and Tiels et al.
  ...
  positionAlloc->Add (Vector (0.0, 0.0, 0.0));		// Position RSU (x,y,z)
  uint32_t vehiclesPosition = 1;
  uint32_t distanceVehicle = 0;

  while (vehiclesPosition<carsNum){		// Vehicles positions
	  if (vehiclesPosition % 8 == 1){
		  positionAlloc->Add (Vector ((100.0-(distanceVehicle*5)), 5.0, 0.0));
	  }
	  else if(vehiclesPosition % 8 == 2){
		  positionAlloc->Add (Vector ((-100.0 +(distanceVehicle*5)), -5.0, 0.0));
	  }
	  else if(vehiclesPosition % 8 == 3){
		  positionAlloc->Add (Vector ((95.0-(distanceVehicle*5)), 10.0, 0.0));
	  }
	  else if(vehiclesPosition % 8 == 4){
		  positionAlloc->Add (Vector ((-95.0 +(distanceVehicle*5)), -10.0, 0.0));
	  }
	  else if(vehiclesPosition % 8 == 5){
		  positionAlloc->Add (Vector ((90.0-(distanceVehicle*5)), 15.0, 0.0));
	  }
	  else if(vehiclesPosition % 8 == 6){
	  	  positionAlloc->Add (Vector ((-90.0+(distanceVehicle*5)), -15.0, 0.0));
	  }
	  else if(vehiclesPosition % 8 == 7){
	  	  positionAlloc->Add (Vector ((85.0-(distanceVehicle*5)), 20.0, 0.0));
	  }
	  else if(vehiclesPosition % 8 == 0){
		  positionAlloc->Add (Vector ((-85.0 +(distanceVehicle*5)), -20.0, 0.0));
		  distanceVehicle++;
	  }
	  vehiclesPosition++;
  ...
  uint32_t vehiclesVelocity = 1;
  Ptr<ConstantVelocityMobilityModel> motion[carsNum];
  while (vehiclesVelocity<carsNum){		// Vehicles velocity
 	  if (vehiclesVelocity % 8 == 1){
 		motion[vehiclesVelocity] = DynamicCast<ConstantVelocityMobilityModel>( cars.Get(vehiclesVelocity)->GetObject<MobilityModel>());
 		motion[vehiclesVelocity]->SetVelocity(Vector(-20.0, 0.0, 0.0));
 	  }
 	  else if(vehiclesVelocity % 8 == 2){
 		motion[vehiclesVelocity] = DynamicCast<ConstantVelocityMobilityModel>( cars.Get(vehiclesVelocity)->GetObject<MobilityModel>());
 		motion[vehiclesVelocity]->SetVelocity(Vector(20.0, 0.0, 0.0));
 	  }
 	  else if(vehiclesVelocity % 8 == 3){
 		motion[vehiclesVelocity] = DynamicCast<ConstantVelocityMobilityModel>( cars.Get(vehiclesVelocity)->GetObject<MobilityModel>());
 		motion[vehiclesVelocity]->SetVelocity(Vector(-15.0, 0.0, 0.0));
 	  }
 	  else if(vehiclesVelocity % 8 == 4){
 		motion[vehiclesVelocity] = DynamicCast<ConstantVelocityMobilityModel>( cars.Get(vehiclesVelocity)->GetObject<MobilityModel>());
 		motion[vehiclesVelocity]->SetVelocity(Vector(15.0, 0.0, 0.0));
 	  }
 	  else if(vehiclesVelocity % 8 == 5){
 		motion[vehiclesVelocity] = DynamicCast<ConstantVelocityMobilityModel>( cars.Get(vehiclesVelocity)->GetObject<MobilityModel>());
 		motion[vehiclesVelocity]->SetVelocity(Vector(-10.0, 0.0, 0.0));
 	  }
 	  else if(vehiclesVelocity % 8 == 6){
 		motion[vehiclesVelocity] = DynamicCast<ConstantVelocityMobilityModel>( cars.Get(vehiclesVelocity)->GetObject<MobilityModel>());
 		motion[vehiclesVelocity]->SetVelocity(Vector(10.0, 0.0, 0.0));
 	  }
 	  else if(vehiclesVelocity % 8 == 7){
 	  	motion[vehiclesVelocity] = DynamicCast<ConstantVelocityMobilityModel>( cars.Get(vehiclesVelocity)->GetObject<MobilityModel>());
 	  	motion[vehiclesVelocity]->SetVelocity(Vector(-5.0, 0.0, 0.0));
 	  }
 	  else if(vehiclesVelocity % 8 == 0){
 	 	motion[vehiclesVelocity] = DynamicCast<ConstantVelocityMobilityModel>( cars.Get(vehiclesVelocity)->GetObject<MobilityModel>());
 	 	motion[vehiclesVelocity]->SetVelocity(Vector(5.0, 0.0, 0.0));
 	  }
 	 vehiclesVelocity++;
